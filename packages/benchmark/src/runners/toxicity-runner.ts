...
import {
  toxicityDataset,
  toxicityDatasetByCategory,
} from '../datasets/toxicity-dataset';
...
/**
 * Run the toxicity detection benchmark
 */
export async function runToxicityBenchmark(
  detect: DetectorFunction,
  detectorName: string = 'Toxicity Detector',
): Promise<BenchmarkResult> {
  const startTime = Date.now();
  const details: TestCaseResult[] = [];

  for (const entry of toxicityDataset) {
  ...
  for (const [category, entries] of Object.entries(toxicityDatasetByCategory)) {
  ...
  return {
    ...
    overall,
    categories,
    totalCases: toxicityDataset.length,
