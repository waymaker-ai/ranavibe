# @ranavibe/multi-tenant

Multi-tenant policy server for teams. HTTP API for managing RANA guardrail policies per-project with tenant isolation, API key authentication, rate limiting, and usage tracking.

## Installation

```bash
npm install @ranavibe/multi-tenant
```

## Quick Start

```typescript
import { TenantServer } from '@ranavibe/multi-tenant';
...
```typescript
import { TenantManager, MemoryStorage } from '@ranavibe/multi-tenant';
