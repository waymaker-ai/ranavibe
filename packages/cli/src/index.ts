...
program
  ...
  .action(async (options) => {
    ...
    const spinner = ora('Creating .aicofounder.yml...').start();
    ...
    try {
      ...
      const configPath = path.join(options.output, '.aicofounder.yml');
      fs.writeFileSync(configPath, customConfig, 'utf8');

      spinner.succeed(chalk.green('Created .aicofounder.yml'));

      console.log(chalk.green.bold('\n✅ CoFounder project initialized!\n'));
      console.log(chalk.gray('Next steps:'));
      console.log(chalk.gray('  1. Customize .aicofounder.yml for your project'));
      console.log(chalk.gray('  2. Run `aicofounder validate` to verify configuration'));
      console.log(chalk.gray('  3. Run `aicofounder check pre` before implementing features\n'));
    } catch (error) {
      spinner.fail(chalk.red('Failed to create .aicofounder.yml'));
...
program
  ...
  .option('-c, --config <path>', 'Path to .aicofounder.yml file')
  .action(async (options) => {
    ...
    try {
      ...
      if (!configPath) {
        spinner.fail(chalk.red('No .aicofounder.yml file found'));
        console.log(chalk.gray('\nRun `aicofounder init` to create one.\n'));
        process.exit(1);
      }
...
program
  ...
  .action(async (phase, options) => {
    ...
    try {
      const configPath = options.config || ConfigParser.findConfig();
      if (!configPath) {
        console.error(chalk.red('No .aicofounder.yml file found'));
        process.exit(1);
      }
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (isMajor) {
        ...
        console.log(chalk.gray('Run: aicofounder repm\n'));
      } else {
        console.log(chalk.green.bold('\n✅ This is a standard feature\n'));
        console.log(chalk.gray('No REPM validation needed.'));
        console.log(chalk.white.bold('\nProceed with regular quality gates:\n'));
        console.log(chalk.gray('  1. Run: aicofounder check pre'));
        console.log(chalk.gray('  2. Implement feature'));
        console.log(chalk.gray('  3. Run: aicofounder check impl'));
        console.log(chalk.gray('  4. Run: aicofounder check test'));
        console.log(chalk.gray('  5. Run: aicofounder check deploy\n'));
      }
    } catch (error) {
      console.error(chalk.red(error instanceof Error ? error.message : String(error)));
      process.exit(1);
  ...
  .action(async (options) => {
    try {
      ...
      if (!config.standards?.design_system?.enabled) {
        console.log(chalk.yellow('⚠️  Design system not enabled in .aicofounder.yml\n'));
        console.log(chalk.gray('Add this to your .aicofounder.yml:\n'));
      ...
      console.log(chalk.gray('Run `aicofounder design-violations --fix` to auto-fix some issues\n'));
