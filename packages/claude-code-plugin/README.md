...
| `hooks/pre-edit-guardrails.sh` | Block secrets, unignored `.env*` writes, and out-of-VibeSpec-scope edits at `Edit`/`Write`/`MultiEdit` time; passively records session-scoped taint on allowed writes containing secrets/PII |
