...
`));
  ...
  if (!ready) {
    console.log(chalk.gray('\nWizard cancelled. Run `cofounder wizard` when you\'re ready.\n'));
    process.exit(0);
  }
}
...
async function stepComplete(state: WizardState) {
  ...
  const tasks = [
    { name: 'Creating .cofounder.yml configuration', done: false },
...
`));
  ...
  console.log(chalk.gray('  .cofounder.yml              - CoFounder configuration'));
  ...
  console.log(chalk.cyan('  3. cofounder check          ') + chalk.gray('# Verify setup'));
  ...
  console.log(chalk.bold.white('\n📚 Helpful Commands:\n'));
  console.log(chalk.gray('  cofounder llm:analyze       - Analyze LLM costs'));
  console.log(chalk.gray('  cofounder security:audit    - Run security audit'));
  console.log(chalk.gray('  cofounder prompt:create     - Create a prompt'));
  console.log(chalk.gray('  cofounder dashboard         - View cost dashboard'));
...
`;
  ...
  fs.writeFileSync('.cofounder.yml', config);
}
