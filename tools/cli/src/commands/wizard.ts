...
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║   🐟  CoFounder Wizard - Build Your AI App in 10 Minutes       ║
║                                                           ║
...
`));
  ...
  if (!ready) {
    console.log(chalk.gray('\nWizard cancelled. Run `aicofounder wizard` when you\'re ready.\n'));
    process.exit(0);
  }
}
...
async function stepComplete(state: WizardState) {
  ...
  const tasks = [
    { name: 'Creating .aicofounder.yml configuration', done: false },
  ...
  // Task 1: Create .aicofounder.yml
  await sleep(300);
  createCoFounderConfig(state);
  tasks[0].done = true;
  printTasks(tasks);
...
`));
  ...
  console.log(chalk.gray('  .aicofounder.yml              - CoFounder configuration'));
  ...
  console.log(chalk.cyan('  3. aicofounder check          ') + chalk.gray('# Verify setup'));
  ...
  console.log(chalk.bold.white('\n📚 Helpful Commands:\n'));
  console.log(chalk.gray('  aicofounder llm:analyze       - Analyze LLM costs'));
  console.log(chalk.gray('  aicofounder security:audit    - Run security audit'));
  console.log(chalk.gray('  aicofounder prompt:create     - Create a prompt'));
  console.log(chalk.gray('  aicofounder dashboard         - View cost dashboard'));

  console.log(chalk.gray('\n─'.repeat(60)));
  console.log(chalk.gray('  Documentation: https://cofounder.dev'));
  console.log(chalk.gray('  Support: https://discord.gg/cofounder\n'));
}
...
function createCoFounderConfig(state: WizardState) {
  const config = `# CoFounder Configuration
# Generated by \`aicofounder wizard\`
...
`;
  ...
  fs.writeFileSync('.aicofounder.yml', config);
}

function createLLMClient(state: WizardState) {
  const clientCode = `/**
 * LLM Client
 * Generated by CoFounder Wizard
 */
