...
export async function dockerBuild(options: {
  ...
  // Check for .cofounder/docker.json config
  let registry = '';
  ...
  try {
    ...
    if (options.push) {
      ...
      console.log(chalk.cyan(`  aicofounder docker:push --tag ${tag}\n`));
    }
...
async function createDockerfile() {
  ...
  if (hasNextJs) {
    dockerfile = `# CoFounder Dockerfile for Next.js
# Generated by \`aicofounder docker:build\`
...
`;
    ...
    dockerfile = `# CoFounder Dockerfile for Node.js
# Generated by \`aicofounder docker:build\`
...
`;
  ...
  if (!fs.existsSync('.dockerignore')) {
    const dockerignore = `# CoFounder .dockerignore
# Generated by \`aicofounder docker:build\`
...
export async function dockerCompose(options: {
  ...
  if (!fs.existsSync('docker-compose.yml') && !fs.existsSync('docker-compose.yaml')) {
    ...
    console.log(chalk.gray('Create one or use the CoFounder template:\n'));
    console.log(chalk.cyan('  aicofounder init --template docker\n'));
    return;
  }
