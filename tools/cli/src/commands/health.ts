...
export async function healthCheck(options: {
  ...
  if (results.some((r) => r.status === 'unknown' && r.endpoint.includes('/api/health'))) {
    console.log(chalk.gray('Tip: Run `cofounder health:setup` to add health endpoints to your app.\n'));
  }
}
...
`);
  ...
  console.log(chalk.cyan('  cofounder health:check\n'));
}
...
function generateMonitorIntegration(config: MonitorConfig): string {
  return `/**
 * Monitor Integration
 * Generated by \`cofounder monitor:setup\`
 */
...
export async function monitorStatus() {
  ...
  if (!fs.existsSync(configFile)) {
    ...
    console.log(chalk.gray('\nRun `cofounder monitor:setup` to configure monitoring.\n'));
    return;
  }
