...
export async function healthCheck(options: {
  ...
  if (fs.existsSync('.cofounder/health.json')) {
    try {
      const config = JSON.parse(fs.readFileSync('.cofounder/health.json', 'utf-8'));
      if (config.endpoints) {
  ...
  if (results.some((r) => r.status === 'unknown' && r.endpoint.includes('/api/health'))) {
    console.log(chalk.gray('Tip: Run `aicofounder health:setup` to add health endpoints to your app.\n'));
  }
}
...
export async function healthSetup() {
  console.log(chalk.bold.cyan('\n🏥 CoFounder Health Endpoint Setup\n'));
...
`);
  ...
  console.log(chalk.cyan('  aicofounder health:check\n'));
}
...
export async function monitorSetup() {
  ...
  fs.writeFileSync('.cofounder/monitor-integration.ts', integrationCode);

  console.log(chalk.green('\n✅ Monitoring configured!\n'));
  console.log(chalk.gray('Files created:'));
  console.log(chalk.gray('  - .cofounder/monitor.json'));
  console.log(chalk.gray('  - .cofounder/monitor-integration.ts'));
  console.log();
...
function generateMonitorIntegration(config: MonitorConfig): string {
  return `/**
 * Monitor Integration
 * Generated by \`aicofounder monitor:setup\`
 */
...
export async function monitorStatus() {
  ...
  const configFile = '.cofounder/monitor.json';

  if (!fs.existsSync(configFile)) {
    console.log(chalk.yellow('No monitoring configured.'));
    console.log(chalk.gray('\nRun `aicofounder monitor:setup` to configure monitoring.\n'));
    return;
  }
