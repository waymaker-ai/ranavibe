/**
 * Prompt CLI Commands
 * Surfaces @waymakerai/aicofounder-prompts package functionality via CLI
 */
...
const PROMPTS_FILE = '.cofounder/prompts.json';
const PROMPTS_DIR = '.cofounder/prompts';
...
export async function promptCreate(options: {
  ...
  console.log(chalk.cyan(`  aicofounder prompt:test ${id}`));
  console.log();
}
...
export async function promptList(options: { verbose?: boolean } = {}) {
  ...
  if (promptsList.length === 0) {
    ...
    console.log(chalk.cyan('  aicofounder prompt:create'));
    console.log();
    return;
  }
...
export async function promptTest(promptId: string, options: {
  ...
  if (options.dryRun) {
    ...
    if (execute) {
      ...
      console.log(chalk.gray('Run `aicofounder llm:setup` to configure providers.\n'));
    }
  }
...
export async function promptProtect(options: {
  ...
  console.log(chalk.bold.cyan('\n🔒 CoFounder Prompt Protection\n'));

  const configFile = '.cofounder/protection.json';
  let config: {
    enabled: boolean;
 ...
 * CoFounder Prompt Protection Middleware
 * Generated by \`aicofounder prompt:protect\`
 */
...
`;
  ...
  fs.writeFileSync('.cofounder/protection-middleware.ts', middlewareCode);

  console.log(chalk.green('\n✅ Prompt protection configured!\n'));
  console.log(chalk.gray('Files created:'));
  console.log(chalk.gray(`  - ${configFile}`));
  console.log(chalk.gray(`  - .cofounder/protection-middleware.ts`));
  console.log();
  console.log(chalk.gray('Usage in your code:'));
  console.log(chalk.cyan(`  import { protectPrompt } from './.cofounder/protection-middleware';`));
...
export async function promptTemplates() {
  ...
  console.log(chalk.cyan('  aicofounder prompt:create --id my-prompt --template "Your template here"'));
  console.log();
}
