 ...
 * ```bash
 * # Generate a chatbot
 * cofounder new chatbot support-bot
 *
 * # Generate a RAG system
 * cofounder new rag docs-search
 *
 * # Generate an AI agent
 * cofounder new agent data-analyst
 *
 * # Generate an API endpoint
 * cofounder new api chat
 * ```
 */
...
export async function newCommand(
  ...
  if (!validTypes.includes(type)) {
    ...
    console.log(chalk.gray('\nExample: cofounder new chatbot support-bot\n'));
    return;
  }
...
`,
    ...
    dependencies: ['@aicofounder/core', '@aicofounder/testing'],
    postCreate: [
      '1. Configure your API key: cofounder config:set --provider openai',
      '2. Import and use: import { ' + camelName + " } from './src/" + safeName + "'",
      '3. Run tests: cofounder test',
    ],
  };
}
...
import { RAGPresets, type RAGPipeline } from '@aicofounder/rag';
...
`,
    ...
    dependencies: ['@aicofounder/core', '@aicofounder/rag', '@aicofounder/testing'],
    postCreate: [
      '1. Configure your API key: cofounder config:set --provider openai',
