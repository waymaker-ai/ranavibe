/**
 * CoFounder New Command - Scaffolding Generator
 *
 ...
 * ```bash
 * # Generate a chatbot
 * aicofounder new chatbot support-bot
 *
 * # Generate a RAG system
 * aicofounder new rag docs-search
 *
 * # Generate an AI agent
 * aicofounder new agent data-analyst
 *
 * # Generate an API endpoint
 * aicofounder new api chat
 * ```
 */
...
export async function newCommand(
  ...
  if (!validTypes.includes(type)) {
    ...
    console.log(chalk.gray('\nExample: aicofounder new chatbot support-bot\n'));
    return;
  }
...
import { describe, aiTest } from '@waymakerai/aicofounder-testing';
...
`,
    ...
    dependencies: ['@waymakerai/aicofounder-core', '@waymakerai/aicofounder-testing'],
    postCreate: [
      '1. Configure your API key: aicofounder config:set --provider openai',
      '2. Import and use: import { ' + camelName + " } from './src/" + safeName + "'",
      '3. Run tests: aicofounder test',
    ],
  };
}
...
import { RAGPresets, type RAGPipeline } from '@waymakerai/aicofounder-rag';
...
describe('${pascalName} RAG', () => {
  aiTest('should answer questions from indexed documents', async ({ expect }) => {
    ...
    await rag.index([
      { content: 'CoFounder supports 9 different LLM providers.', metadata: { source: 'docs' } },
      { content: 'Caching can reduce costs by up to 70%.', metadata: { source: 'docs' } },
    ]);

    const result = await rag.query('How many providers does CoFounder support?');
...
`,
    ...
    dependencies: ['@waymakerai/aicofounder-core', '@waymakerai/aicofounder-rag', '@waymakerai/aicofounder-testing'],
    postCreate: [
      '1. Configure your API key: aicofounder config:set --provider openai',
...
const client = new CoFounderClient({
  cache: {
    type: 'memory',
...
`,
    ...
    dependencies: ['@waymakerai/aicofounder-core'],
    postCreate: [
      '1. Configure your API key: aicofounder config:set --provider openai',
