 ...
 * aicofounder completion
 *
 * # Generate for specific shell
 * aicofounder completion:bash >> ~/.bashrc
 * aicofounder completion:zsh >> ~/.zshrc
 * aicofounder completion:fish > ~/.config/fish/completions/cofounder.fish
 *
 * # Install completions automatically
 * aicofounder completion:install
 * ```
 */
...
// All CoFounder CLI commands with descriptions
const COMMANDS = [
  // Core commands
  { name: 'init', desc: 'Initialize CoFounder in your project' },
  { name: 'check', desc: 'Check compliance with CoFounder standards' },
  { name: 'deploy', desc: 'Deploy with CoFounder verification workflow' },
  { name: 'deploy:status', desc: 'Show deployment history' },
  { name: 'deploy:rollback', desc: 'Rollback to previous deployment' },
  { name: 'validate', desc: 'Validate .aicofounder.yml configuration' },
  { name: 'status', desc: 'Show CoFounder project status' },
...
export async function completionCommand(): Promise<void> {
  ...
  console.log(chalk.gray('  aicofounder completion:bash >> ~/.bashrc'));
  console.log(chalk.gray('  source ~/.bashrc\n'));

  console.log(chalk.bold('Zsh:'));
  console.log(chalk.gray('  aicofounder completion:zsh >> ~/.zshrc'));
  console.log(chalk.gray('  source ~/.zshrc\n'));

  console.log(chalk.bold('Fish:'));
  console.log(chalk.gray('  aicofounder completion:fish > ~/.config/fish/completions/cofounder.fish\n'));

  console.log(chalk.bold('Or auto-install for your shell:'));
  console.log(chalk.cyan('  aicofounder completion:install\n'));
}
...
export async function completionInstallCommand(): Promise<void> {
  const shell = detectShell();
  console.log(chalk.bold.cyan('\n🎯 Installing CoFounder Completions\n'));
  console.log(`Detected shell: ${chalk.white(shell)}\n`);
  ...
  try {
    switch (shell) {
      ...
      default:
        ...
        console.log(chalk.gray('  aicofounder completion:bash   # for bash'));
        console.log(chalk.gray('  aicofounder completion:zsh    # for zsh'));
        console.log(chalk.gray('  aicofounder completion:fish   # for fish'));
    }
  } catch (error: any) {
...
function generateBashCompletion(): string {
  ...
  return `# CoFounder CLI Bash Completion
# Generated by: aicofounder completion:bash
# Add to ~/.bashrc: source <(aicofounder completion:bash)

_cofounder_completions() {
...
complete -F _cofounder_completions cofounder
`;
}
...
# CoFounder CLI Zsh Completion
# Generated by: aicofounder completion:zsh
# Add to ~/.zshrc: source <(aicofounder completion:zsh)
...
_cofounder() {
    ...
    case \$state in
        command)
            _describe -t commands 'aicofounder commands' commands
            ;;
        args)
            case \$words[2] in
...
function generateFishCompletion(): string {
  const commandCompletions = COMMANDS.map(
    c => `complete -c cofounder -n "__fish_use_subcommand" -a "${c.name}" -d "${c.desc}"`
  ).join('\n');

  const providerCompletions = PROVIDERS.map(
    p => `complete -c cofounder -n "__fish_seen_subcommand_from config:set config:validate config:remove" -a "${p}"`
  ).join('\n');

  const ollamaCompletions = OLLAMA_MODELS.map(
    m => `complete -c cofounder -n "__fish_seen_subcommand_from ollama:pull ollama:test" -a "${m}"`
  ).join('\n');

  const presetCompletions = BUDGET_PRESETS.map(
    p => `complete -c cofounder -n "__fish_seen_subcommand_from budget:preset" -a "${p}"`
  ).join('\n');

  const newCompletions = NEW_TYPES.map(
    t => `complete -c cofounder -n "__fish_seen_subcommand_from new" -a "${t}"`
  ).join('\n');

  const learnCompletions = LEARN_TOPICS.map(
    t => `complete -c cofounder -n "__fish_seen_subcommand_from learn" -a "${t}"`
  ).join('\n');

  return `# CoFounder CLI Fish Completion
# Generated by: aicofounder completion:fish
# Save to: ~/.config/fish/completions/cofounder.fish
...
# Common options
complete -c cofounder -s h -l help -d "Show help"
complete -c cofounder -s V -l version -d "Show version"

# init options
complete -c cofounder -n "__fish_seen_subcommand_from init" -s t -l template -d "Template" -a "default react nextjs vue"
complete -c cofounder -n "__fish_seen_subcommand_from init" -s f -l force -d "Force overwrite"

# deploy options
complete -c cofounder -n "__fish_seen_subcommand_from deploy" -l verify -d "Verify deployment"
complete -c cofounder -n "__fish_seen_subcommand_from deploy" -l skip-tests -d "Skip testing"
complete -c cofounder -n "__fish_seen_subcommand_from deploy" -l skip-build -d "Skip build"
complete -c cofounder -n "__fish_seen_subcommand_from deploy" -l prod -d "Deploy to production"
complete -c cofounder -n "__fish_seen_subcommand_from deploy" -s p -l platform -d "Platform" -a "vercel railway docker"

# test options
complete -c cofounder -n "__fish_seen_subcommand_from test" -s c -l coverage -d "Coverage report"
complete -c cofounder -n "__fish_seen_subcommand_from test" -s w -l watch -d "Watch mode"
complete -c cofounder -n "__fish_seen_subcommand_from test" -s v -l verbose -d "Verbose output"
complete -c cofounder -n "__fish_seen_subcommand_from test" -l update-baselines -d "Update baselines"
complete -c cofounder -n "__fish_seen_subcommand_from test" -l update-snapshots -d "Update snapshots"
complete -c cofounder -n "__fish_seen_subcommand_from test" -l parallel -d "Run parallel"

# budget:set options
complete -c cofounder -n "__fish_seen_subcommand_from budget:set" -s l -l limit -d "Limit amount"
complete -c cofounder -n "__fish_seen_subcommand_from budget:set" -s p -l period -d "Period" -a "hourly daily weekly monthly total"
complete -c cofounder -n "__fish_seen_subcommand_from budget:set" -s a -l action -d "Action" -a "block warn log"
complete -c cofounder -n "__fish_seen_subcommand_from budget:set" -s i -l interactive -d "Interactive mode"
`;
}
...
async function installBashCompletion(): Promise<void> {
  ...
  if (fs.existsSync(bashrc)) {
    ...
    if (content.includes('_cofounder_completions')) {
      console.log(chalk.yellow('Bash completion already installed.'));
      console.log(chalk.gray('To reinstall, remove the CoFounder completion block from ~/.bashrc'));
      return;
    }
  }
...
async function installZshCompletion(): Promise<void> {
  ...
  if (fs.existsSync(zshrc)) {
    ...
    if (content.includes('#compdef cofounder') || content.includes('_cofounder')) {
      console.log(chalk.yellow('Zsh completion already installed.'));
      console.log(chalk.gray('To reinstall, remove the CoFounder completion block from ~/.zshrc'));
      return;
    }
  }
...
async function installFishCompletion(): Promise<void> {
  ...
  const fishFile = path.join(fishDir, 'cofounder.fish');
