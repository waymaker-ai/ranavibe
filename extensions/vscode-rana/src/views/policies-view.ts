...
/**
 * TreeDataProvider for the CoFounder Policies sidebar panel.
...
export class PoliciesViewProvider implements vscode.TreeDataProvider<PolicyNode> {
  ...
  async refreshPolicies(): Promise<void> {
    ...
    try {
      // Look for .cofounder/policies/*.yml in all workspace folders
      const policyGlob = '**/.cofounder/policies/*.{yml,yaml}';
  ...
  async togglePolicy(node: PolicyFileNode): Promise<void> {
    ...
    try {
      ...
      vscode.window.showInformationMessage(
        `CoFounder: Policy "${node.policyName}" ${node.isEnabled ? 'disabled' : 'enabled'}.`
      );
    } catch (err) {
      vscode.window.showErrorMessage(`CoFounder: Failed to toggle policy: ${err}`);
    }
  }
  ...
  async createFromPreset(presetName: string): Promise<void> {
    ...
    const template = [
      `# CoFounder Policy: ${presetName}`,
      `# Generated by CoFounder Guardrails extension`,
      ``,
      `name: ${presetName}`,
      `enabled: true`,
      ``,
    ...
    try {
      ...
      vscode.window.showInformationMessage(`CoFounder: Created policy "${presetName}".`);
    } catch (err) {
      vscode.window.showErrorMessage(`CoFounder: Failed to create policy: ${err}`);
    }
  }
